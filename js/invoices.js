    ...
    function renderInvoices() {
        ...
        filteredInvoices.forEach((inv, index) => {
            ...
            const statusBadge = isPaid 
                ? `<span style="background: rgba(16, 185, 129, 0.1); color: #059669; padding: 6px 14px; border-radius: 20px; font-size: 0.85rem; font-weight: 700; border: 1px solid rgba(16, 185, 129, 0.2); box-shadow: 0 0 10px rgba(16, 185, 129, 0.1);">✓ ชำระเงินแล้ว (PAID)</span>`
                : `<span style="background: rgba(245, 158, 11, 0.1); color: #d97706; padding: 6px 14px; border-radius: 20px; font-size: 0.85rem; font-weight: 700; border: 1px solid rgba(245, 158, 11, 0.2); box-shadow: 0 0 10px rgba(245, 158, 11, 0.1);">⏳ รอการชำระเงิน (PENDING)</span>`;
            ...
            <details class="invoice-accordion" style="background: rgba(255,255,255,0.7); backdrop-filter: blur(10px); border-radius: 16px; border: 1px solid rgba(226, 232, 240, 0.8); overflow: hidden; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); box-shadow: 0 4px 15px rgba(0,0,0,0.03); margin-bottom: 12px; animation: slideUpFade 0.5s ease backwards; animation-delay: ${index * 0.08}s;">
                <summary style="padding: 24px 30px; cursor: pointer; list-style: none; outline: none; background: transparent; display: grid; grid-template-columns: 2.5fr 1fr 1.5fr 1.5fr 40px; align-items: center; gap: 20px;" onmouseover="this.style.background='rgba(255,255,255,0.9)'" onmouseout="this.style.background='transparent'">
                    ...
                    <div style="display: flex; align-items: center; gap: 16px;">
                        <div style="width: 50px; height: 50px; border-radius: 14px; background: linear-gradient(135deg, ${isPaid ? '#d1fae5, #a7f3d0' : '#fef3c7, #fde68a'}); color: ${isPaid ? '#059669' : '#d97706'}; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; box-shadow: 0 4px 10px ${isPaid ? 'rgba(16,185,129,0.1)' : 'rgba(245,158,11,0.1)'};">
                            ${isPaid ? '✓' : '⏳'}
                        </div>
                        <div>
                ...
                <div style="padding: 30px; border-top: 1px dashed #cbd5e1; background: rgba(248, 250, 252, 0.5);">
                    <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 24px;">
                        ...
                        <div style="background: white; border-radius: 16px; box-shadow: 0 10px 25px rgba(0,0,0,0.05); padding: 40px; border: 1px solid #e2e8f0; position: relative; overflow: hidden;">
                            <div style="position: absolute; top: 0; left: 0; width: 100%; height: 6px; background: linear-gradient(90deg, #10b981, #3b82f6, #8b5cf6);"></div>
